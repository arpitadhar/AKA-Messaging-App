...
async fn login(
    ...
    if user_exists {
        ...
        let user_img_url = retrieve_img_url(connection, &payload.email).expect("reason").to_string(); 
...
async fn retrieve_user_information(Json(payload): Json<String>
    ...
    //NEW IMAGE
    let user_img_url = retrieve_img_url(connection, &email_response).expect("reason").to_string(); 
...
async fn update_img(Json(payload): Json<InputEmail>) -> (StatusCode, Json<String>){
    use schema::users::dsl::*; 
    let connection: &mut PgConnection = &mut establish_connection();
    let new_img = &payload.input; 
    let update_row = diesel::update(users.filter(email.eq(&payload.email)))
        .set(img_url.eq(&payload.input));
    println!("{}", diesel::debug_query::<diesel::pg::Pg, _>(&update_row).to_string());
...
//NEW IMAGE
fn retrieve_img_url(conn: &mut PgConnection, email_input: &str) -> Option<String>{
    use schema::users::dsl::*; 
    match users.filter(email.eq(email_input)).select(img_url).first::<String>(conn)  {
        Ok(imgurl_1) => Some(imgurl_1), 
        Err(_) => None,
    }
}
