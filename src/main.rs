...
async fn create_user(
    ...
    let in_token = "a".to_string(); 

    insert_user(connection, &payload.username, &payload.first_name, &payload.last_name, &payload.email, &hashed_password, &payload.token); 
...
fn insert_user(conn2: &mut PgConnection, username_input: &str, first_name_input: &str, last_name_input: &str, email_input: &str, password_input: &str, token: &str) -> Result<usize, diesel::result::Error> {
        use schema::users::dsl::*;
        println!("1"); 
        let rows_inserted = insert_into(users)
            .values((username.eq(username_input), password.eq(password_input), first_name.eq(first_name_input), last_name.eq(last_name_input), email.eq(email_input), token.eq("none")))
            .execute(conn2)?;
